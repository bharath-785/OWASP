...
No cloud API keys required to run this — everything here runs entirely on your own machine, against your own local model, with fake/synthetic data.
...
```
sandboxes/
  prompt-injection-sandbox-ollama.html   — interactive test harness for LLM01 (Prompt Injection)
findings/
  LLM01-prompt-injection.md              — write-up of methodology and results
```

More patterns will be added here as I work through the rest of the list (see Progress below).
...
Then open:
```
http://localhost:8000/prompt-injection-sandbox-ollama.html
```
